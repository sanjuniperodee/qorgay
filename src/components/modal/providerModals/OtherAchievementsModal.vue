...
import { useOtherAchievementsForm } from '@components/forms/providerForms/useOtherAchievementsForm'
    ...
    <div class="modal">
      <form @submit.prevent="handleSubmit">
        <div class="header">
          ...
          <h2 class="modal-title">Другие заслуги</h2>
        </div>

        <div class="statement-list">
          <v-text-field
            v-model="otherAchievements.name"
            label="Название"
          ...
          <div class="year-average">
            <v-text-field
              v-model.number="otherAchievements.year_start"
              label="Год начала"
              type="number"
            ...
            <v-text-field
              v-if="!performingNow"
              v-model.number="otherAchievements.year_end"
              label="Год окончания"
              type="number"
              :rules="[rules.required, rules.isNumber]"
            ></v-text-field>
          ...
          <v-switch
            ...
            label="По нынешнее время"
            inset
          ></v-switch>

          <MediaUpload v-if="!uploadedFile" :title="'Фото/Видео'" :handleFileChange="handleFileChange"/>
