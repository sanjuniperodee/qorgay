<template>
  ...
  <div v-if="selectedChat" class="messages" ref="messagesContainer">
    ...
    >
      ...
      <!-- Опция удаления -->
      <div 
        v-if="message.sender === selectedChat.initiator.id" 
        class="delete-option" 
        :class="{ 'show': selectedMessage === message }"
      >
    ...
    <div v-if="props.selectedComplaint?.status === 'NOT_ANSWERED'" class="archived-block">
      <div class="archived-box" v-if="userStore.acceptedRoles.includes(UserRoleOptions.INSPECTOR)">
        <h2>Уважаемый инспектор</h2>
        <p>Обращаем ваше внимание, что по одной из жалоб статус остается “не отвечено”. Просим вас связаться с заявителем для предоставления ответа или уточнения информации.</p>
      </div>
      <div class="archived-box" v-else>
        <h2>Уважаемый заявитель</h2>
        <p>Обращаем ваше внимание, что по вашей жалобе инспектор пока не предоставил ответ. Мы направили ему напоминание о необходимости обработки вашего обращения.</p>
      </div>
    </div>
  </div>
  ...
  const handleMessageClick = (message: Message) => {
    if (message.sender === props.selectedChat?.id) {
