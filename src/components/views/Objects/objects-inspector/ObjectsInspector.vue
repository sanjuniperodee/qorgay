...
const noteObject = async (id: string) => {
  try {
    await objectStore.saveMyObjects(id)
    snackbarStore.pullSnackbar('Обьект успешно сохранен.', 3000, '#5b9271')
  } catch (error) {
    console.error(error)
    snackbarStore.pullSnackbar('Неудалось создать ваш обьект.', 3000, '#d20f0d')
  }
}
...
<template>
  ...
  <div
    ...
    v-show="!isLoading"
  >
    <div class="inspector__title">Объекты</div>
    <div class="inspector__row">
      ...
      <div class="inspector__subtitle">
        ...
        Безопасности Объекта. Если на этой странице не имеется никаких паспортов, означает что к вам
        еще не прикрепили никакие объекты. Если у вас появились вопросы обратитесь к секции "FAQ"
        или обратить по контактному номеру +77474461966
      </div>
    </div>
    ...
    >
      ...
      <template v-slot:item.actions="{ item }">
        <div class="d-flex align-center ga-5">
          ...
          <button
            class="note-btn"
            @click="noteObject(item.id.toString())"
